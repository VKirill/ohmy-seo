...
export async function runWebmasterIndexingIssues(input: {
  host_id?: string;
  site?: string;
  account?: string;
  ...
  try {
    ...
    const result = await withCache(
      { toolName: "webmaster_indexing_issues", accountId: acc.id, args: canonicalArgs, forceRefresh: input.force_refresh ?? false },
      async () => {
        const accessToken = await getAccessToken(acc.id);
        return getIndexingIssues({ accessToken, webmasterUserId: String(acc.webmaster_user_id), host: hostId! });
      }
    );
