...
export async function runWebmasterIndexingIssues(input: { host_id?: string; site?: string; account?: string }) {
  try {
    ...
    const result = await getIndexingIssues({ accessToken, webmasterUserId: String(acc.webmaster_user_id), host: hostId });
