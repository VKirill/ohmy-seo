...
export async function runWebmasterSiteSummary(input: { host: string; account?: string }) {
  try {
    ...
    const result = await getSiteSummary({ accessToken, webmasterUserId: String(acc.webmaster_user_id), host: input.host });
