...
export interface MutateArgs {
  ...
  change: object;
  confirm?: boolean;
  /** DANGER tools only: requires the acknowledge_live echo and the env flag. */
  danger?: boolean;
 ...
 *
 * confirm:false returns a preview and touches nothing. DANGER writes need the
 * env flag GOOGLE_ADS_ALLOW_LIVE_MUTATIONS=true *and* an acknowledge_live token
 * that echoes both the customer and the resource, so a token copied from an
 * earlier call cannot fire against a different account.
 */
