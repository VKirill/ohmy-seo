...
/**
 * Legacy developer token header.
 *
 * Google sunset developer tokens on 2026-09-09. API access is now attached to
 * the Cloud project that owns the OAuth credentials; the old header is optional
 * and ignored. Keep sending it when configured for backwards compatibility.
 */
...
const ERROR_HINTS: Array<[string, string]> = [
  ...
  ["CLOUD_PROJECT_NOT_APPROVED_FOR_PRODUCTION", "Google Cloud project не одобрен для боевых аккаунтов. Запросите Explorer/Basic access в Google Cloud Console."],
...
async function callOnce(
  ...
  const legacyDeveloperToken = developerToken();
...
/** Runs a metadata query through GoogleAdsFieldService, which has no customer path or FROM clause. */
export async function adsFieldsSearch(
  account: AccountRow,
  query: string,
  ...
  const rows = (result.data as { results?: unknown[] } | undefined)?.results ?? [];
  return { ok: true, status: result.status, rows, row_count: rows.length };
}
