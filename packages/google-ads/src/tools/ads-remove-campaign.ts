...
export const TOOL_NAME = "ads_remove_campaign";

/** DANGER and irreversible: removes a campaign. Prefer ads_pause_campaign. */
export async function runAdsRemoveCampaign(args: {
  account?: string;
  customer_id: string;
  campaign_id: string;
  ...
  return runMutation({
    ...
    change: {
      action: "REMOVE",
      irreversible: true,
      alternative: "ads_pause_campaign останавливает показ без удаления истории",
    },
    danger: true,
