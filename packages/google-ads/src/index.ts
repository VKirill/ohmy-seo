...
const server = new McpServer(
  { name: "mcp-google-ads", version: pkg.version },
  {
    instructions:
      ...
      "для выполнения нужны confirm:true и GOOGLE_ADS_ALLOW_LIVE_MUTATIONS=true. " +
      "Опасные операции (включение и пауза кампании, смена бюджета, применение " +
      "рекомендаций и удаление) дополнительно требуют acknowledge_live вида " +
...
reg("delete_google_oauth_app",
  { title: "Google OAuth — Delete App",
    ...
    inputSchema: { app_label: LBL.describe("Label of the app to delete") }, annotations: WRITE },
...
reg("delete_google_account",
  { title: "Google OAuth — Delete Account",
    ...
    inputSchema: { account_label: LBL }, annotations: WRITE },
...
reg("ads_apply_recommendation",
  { title: "Ads — Apply Recommendation (DANGER)",
    description: "Applies one recommendation by resource_name. Recommendations may change spend or delivery, so confirm, acknowledge_live and the global live-mutation flag are required.",
    inputSchema: {
      account: ACCT, customer_id: CID,
