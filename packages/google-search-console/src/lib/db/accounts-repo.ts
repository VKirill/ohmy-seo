...
const PKG = "google-search-console";
...
export type GoogleAccountRow = {
  ...
  access_token: string | null;       // plaintext (decrypted)
  refresh_token: string | null;      // plaintext (decrypted)
  service_account_json: string | null; // plaintext (decrypted)
  expires_at: number;
...
function dbRowToFull(row: AccountDbRow): GoogleAccountRow {
  return {
    ...dbRowToPublic(row),
    access_token: row.access_token_enc ? decryptSecret(row.access_token_enc) : null,
    refresh_token: row.refresh_token_enc ? decryptSecret(row.refresh_token_enc) : null,
