...
const TOOL_NAME = "ga4_list_custom_dimensions";
...
export async function runGa4ListCustomDimensions(args: { account?: string; property: string }) {
  try {
    ...
    const data = result.data as { customDimensions?: unknown[] };
    return {
      content: [
        {
          type: "text" as const,
          text: JSON.stringify(
            { custom_dimensions: data.customDimensions ?? [] },
            null,
            2
          ),
        },
