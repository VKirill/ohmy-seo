...
const PKG_NAME = "gtm";
const TOOL_NAME = "gtm_list_variables";
...
export async function runGtmListVariables(args: {
  account?: string;
  accountId: string;
  containerId: string;
  workspaceId: string;
  ...
  return withCache<{ variables: unknown[] }>(
    {
      toolName: TOOL_NAME,
    ...
    async () => {
      ...
      const variables: unknown[] =
        (result.data as Record<string, unknown>)?.variable as unknown[] ?? [];
