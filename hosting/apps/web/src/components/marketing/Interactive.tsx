...
const scenarios = [
  { label: 'Изменения', question: 'Отправь обновлённую страницу на переобход в Яндекс Вебмастер.', source: 'Яндекс Вебмастер · учебный сайт example.ru', steps: ['Найти сайт в подключённом аккаунте', 'Проверить адрес страницы и доступный лимит', 'Показать операцию перед отправкой'], columns: ['Сайт', 'Страница', 'Операция'], rows: [['example.ru', '/services', 'Переобход']], finding: 'Готова отправка example.ru/services. Подтвердите именно этот адрес. Переобход не гарантирует индексацию.', href: '/integrations/yandex-webmaster', confirmation: 'Подтвердить в примере', completed: 'Пример результата: запрос на переобход принят. Теперь можно проверить его статус. Реальная отправка не выполнялась.' },
...
export function ScenarioDemo() {
  ...
  return <div className="demo-shell">
    ...
    <div className="demo-tabs" aria-label="Выберите пример задачи">{scenarios.map((s, i) => <button key={s.label} type="button" aria-pressed={i === active} onClick={() => { setActive(i); setConfirmed(false); }}>{s.label}</button>)}</div>
    <div className="demo-content" aria-live="polite"><div className="demo-question"><span>Вы</span><p>{item.question}</p></div>
      <div className="demo-answer"><span className="demo-avatar" aria-hidden="true">AI</span><div><strong>{item.confirmation ? 'Подготовка операции' : 'Проверка данных'}</strong><ol>{item.steps.map(s => <li key={s}>{s}</li>)}</ol></div></div>
      ...
      <p className="demo-finding">{item.finding}</p>{item.confirmation && <div className="demo-confirmation">{confirmed ? <p role="status">{item.completed}</p> : <button type="button" className="mk-button secondary" onClick={() => setConfirmed(true)}>{item.confirmation}</button>}</div>}<a className="text-link" href={item.href}>Как работает интеграция <span aria-hidden="true">↗</span></a>
